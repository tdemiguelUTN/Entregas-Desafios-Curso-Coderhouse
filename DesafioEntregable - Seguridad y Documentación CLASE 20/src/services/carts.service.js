...
class CartsService {
    async findAll() {
        ...
        if (response == null) {
            throw new CustomeError( ErrorMessages.CARTS_NOT_FOUND, 404);
        }
        return response;
    }
    ...
    async findById(cId) {
        ...
        if (response == null) {
            throw new CustomeError( ErrorMessages.CART_NOT_FOUND, 404);
        }
        return response;
    }
    ...
    async addProductCart(idCart, idProduct, user) {
        ...
        if (!cart) throw new CustomeError( ErrorMessages.CART_NOT_FOUND, 404);
        if (!product) throw new CustomeError( ErrorMessages.PRODUCTS_NOT_FOUND, 404);

        if (user.email == product.owner){
        throw new CustomeError( ErrorMessages.ADD_PRODUCT_TO_CART_ERROR, 409);
        }
    ...
    async updateProduct(idCart, idProduct, quantity) {
        ...
        if (!productToUpdate) throw new CustomeError( ErrorMessages.PRODUCT_NOT_FOUND_IN_CART, 404);
    ...
    async deleteAllProductsFromCart(idCart) {
        const cart = await cartsManager.findById(idCart);
        if (cart == null) throw new CustomeError( ErrorMessages.CART_NOT_FOUND, 404);
    ...
    async deleteProductFromCart(idCart, idProduct) {
        ...
        if (!finalCart) throw new CustomeError(ErrorMessages.PRODUCT_NOT_FOUND_IN_CART, 404); 
